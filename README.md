...
| tag.*your_tag* | <p>Glob paths to your scripts that you want to apply the tag "your_tag" on. To declare multiple tags, you will have to add multiple properties in your settings. A tag ```my_tag``` will have as as property name ```tag.my_tag```</p> <p>Tags can be absolute paths or relative paths, relative paths are treated as relative to `scan.path`.</p> <p>**WARNING:** alias from your environment won't do the trick for example tilde (~) is not resolve to your home path.</p> |  | no | ```[data/*, script1.js, old/old_script1.js]``` |
